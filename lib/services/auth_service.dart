...
class AuthService {
  ...
  Future<Map<String, dynamic>?> updateProfilePhoto(String token, String imagePath) async {
    try {
      var request = http.MultipartRequest('POST', Uri.parse('$baseUrl/user/update-photo'));
  ...
  Future<Map<String, dynamic>?> updateProfileInfo(String token, String name) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final error = jsonDecode(response.body);
        throw error['message'] ?? 'Erreur lors de la mise à jour du profil';
      }
    } catch (e) {
      rethrow;
    }
  ...
  Future<Map<String, dynamic>?> deleteProfilePhoto(String token) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final error = jsonDecode(response.body);
        throw error['message'] ?? 'Erreur lors de la suppression';
      }
    } catch (e) {
      rethrow;
    }
  ...
  Future<Map<String, dynamic>?> loginWithGoogle(String googleToken) async {
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        final error = jsonDecode(response.body);
        throw error['message'] ?? 'Erreur de connexion Google';
      }
    } catch (e) {
      rethrow;
    }
